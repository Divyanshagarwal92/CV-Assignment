yfunction stitched_img = stitchImg(varargin)

imnum = size(varargin,2);
imgd = cell2mat(varargin(1));   

for i=2:imnum-1

imgs = cell2mat(varargin(i));
%imshow(imgd);
%figure, imshow(imgs);
[rd cd dd] = size(imgd);
[rs cs ds] = size(imgs);
...
% stitched_img = stitched_img .* cat(3, masks, masks, masks) + dest_img;
imgd = stitched_img;
...
%plot(source_bounding_pts(:,1), source_bounding_pts(:,2));
%plot(src_new_bounding_pts(:,1), src_new_bounding_pts(:,2));